...
def write_bids_dataset(
    ...
    *,
    ...
    reference: str = "Cz",
    name: str = "Synthetic EXG sample",
) -> Path:
    """Write a minimal but valid BIDS EEG dataset, in BrainVision format.

    Used to exercise the BIDS reader without needing a real dataset on hand.
    The layout follows the specification: the signal as three BrainVision
    files, plus the sidecars that say what the channels are and when the
    events happened.

    Samples are stored as float32, which BrainVision permits and which avoids
    having to pick a fixed-point resolution per channel.
    """
    root = Path(root).expanduser().resolve()
    eeg_dir = root / f"sub-{subject}" / "eeg"
    eeg_dir.mkdir(parents=True, exist_ok=True)
    stem = f"sub-{subject}_task-{task}"

    n_channels, n_samples = data_uv.shape
    channel_types = channel_types or ["EEG"] * n_channels

    # The samples, multiplexed: all channels for sample 0, then sample 1, ...
    data_uv.T.astype("<f4").tofile(eeg_dir / f"{stem}_eeg.eeg")

    # The header. SamplingInterval is in microseconds, which is how the format
    # states the rate.
    header = [
    ...
    # "Ch<n>=name,reference,resolution,unit". A resolution of 1 with float
    # storage means the stored value is already in the stated unit.
    header += [f"Ch{i + 1}={ch},,1,µV" for i, ch in enumerate(channels)]
    (eeg_dir / f"{stem}_eeg.vhdr").write_text("\n".join(header) + "\n", encoding="utf-8")

    # A marker file with just the segment start. In BIDS the real events live
    # in events.tsv, but the format expects this file to exist.
    ...
    # The required sidecar.
    json.dump(
        {
            "TaskName": task,
            "SamplingFrequency": float(fs),
            "EEGReference": reference,
            "PowerLineFrequency": float(power_line_hz),
            "SoftwareFilters": "n/a",
            "EEGChannelCount": sum(1 for t in channel_types if t == "EEG"),
            "RecordingDuration": round(n_samples / fs, 3),
            "RecordingType": "continuous",
        },
        open(eeg_dir / f"{stem}_eeg.json", "w"),
        indent=2,
    )

    # One row per channel: name, type and units are all required.
    with open(eeg_dir / f"{stem}_channels.tsv", "w", encoding="utf-8") as handle:
        handle.write("name\ttype\tunits\n")
        for channel, kind in zip(channels, channel_types):
            handle.write(f"{channel}\t{kind}\tµV\n")

    # Events: onset and duration in seconds, with the sample index alongside.
    with open(eeg_dir / f"{stem}_events.tsv", "w", encoding="utf-8") as handle:
        handle.write("onset\tduration\ttrial_type\tsample\n")
        for label, onset, duration in sorted(markers, key=lambda m: m[1]):
            handle.write(f"{onset:.4f}\t{duration:.4f}\t{label}\t{int(round(onset * fs))}\n")

    json.dump(
        {"Name": name, "BIDSVersion": "1.9.0", "DatasetType": "raw",
         "Authors": ["Anthriq Research Team"]},
        open(root / "dataset_description.json", "w"),
        indent=2,
    )
    (root / "participants.tsv").write_text(
        f"participant_id\tage\tsex\nsub-{subject}\tn/a\tn/a\n", encoding="utf-8"
    )
    (root / "README").write_text(
        f"{name}\n\nGenerated by scripts/synth.py. Not a recording of a person.\n",
        encoding="utf-8",
    )
    return root
...
def make_alpha(
    ...
    *,
    ...
    if layout == "bids":
        folder = write_bids_dataset(out, data, fs, channels, markers, task="alpha")
    else:
...
def make_ssvep(
    ...
    *,
    ...
    if layout == "bids":
        folder = write_bids_dataset(out, data, fs, channels, markers, task="ssvep")
    else:
        folder = write_bxi_export(out, data, fs, channels, markers)
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_alpha.add_argument("--layout", choices=["bxi", "bids"], default="bxi",
                         help="Output format. bids writes a BIDS dataset in BrainVision form.")
    ...
    p_ssvep.add_argument("--layout", choices=["bxi", "bids"], default="bxi",
                         help="Output format. bids writes a BIDS dataset in BrainVision form.")
    ...
    if args.kind == "alpha":
        path, truth = make_alpha(
            ...
            noise_uv=args.noise, layout=args.layout, seed=args.seed,
        )
